...
function reply_to_read($atts, $content=null) {
    extract(shortcode_atts(array("notice" => '<p class="warningbox reply-to-read">'.__('<span style="color:red; font-size=13px;">温馨提示：</span>此处内容需要<a href="#respond" title="评论本文">评论本文</a>后才能查看。','salong').'</p>'), $atts));
    $email = null;
...
function sl_related_posts( $atts ) {
	...
	foreach ($tags as $tag) {
        ...
        $related .= '<h2><i class="icon-th-list"></i>'.sprintf( __( '<a href="%s" title="查看“%s”标签的相关文章列表">“%s”的相关文章</a>' , 'salong' ), esc_attr(get_tag_link($tag)), esc_attr($tag->name), esc_attr($tag->name)).'</h2>';
        $related .='<ul>';
