...
if ( !comments_open($comment_post_ID) ) {
    ...
    err(__('抱歉，评论已关闭。','salong')); // 将 wp_die 改为错误提示
} elseif ( 'trash' == $status ) {
    do_action('comment_on_trash', $comment_post_ID);
    err(__('无效的评论状态。','salong')); // 将 exit 改为错误提示
} elseif ( !$status_obj->public && !$status_obj->private ) {
    do_action('comment_on_draft', $comment_post_ID);
    err(__('无效的评论状态。','salong')); // 将 exit 改为错误提示
} elseif ( post_password_required($comment_post_ID) ) {
    do_action('comment_on_password_protected', $comment_post_ID);
    err(__('密码保护','salong')); // 将 exit 改为错误提示
} else {
...
if ( $user->ID ) {
    ...
    if ( get_option('comment_registration') || 'private' == $status )
        err(__('抱歉，必须登录才能发表评论！','salong')); // 将 wp_die 改为错误提示
}
...
if ( get_option('require_name_email') && !$user->ID ) {
    if ( 6 > strlen($comment_author_email) || '' == $comment_author )
        err( __('错误：请输入所需的字段（用户名、邮箱）。','salong') ); // 将 wp_die 改为错误提示
    elseif ( !is_email($comment_author_email))
        err( __('错误，请输入一个有效的邮箱地址。','salong') );// 将 wp_die 改为错误提示
}

if ( '' == $comment_content )
    err( __('错误：请输入评论。','salong') ); // 将 wp_die 改为错误提示
...
?>
    <li <?php comment_class(); ?> id="li-comment-<?php comment_ID(); ?>">
        ...
        <div class="comment-author">
            ...
            <?php printf( __( '<span class="comment-info">%s</span> <span class="says">回复:</span>'), get_comment_author_link() ); ?>
