...
class WC_Alipay extends WC_Payment_Gateway {
    ...
    function init_form_fields() {
        ...
        $this->form_fields = array(
            ...
            'partnerID' => array(
                'title'       => __('商户ID', 'alipay'),
                'type'        => 'text',
                'description' => __('请输入您的商户ID<br />如果您还没有商户ID，请<a href="https://www.mobanzhu.com/go/aHR0cDovL3dwYS5xcS5jb20vbXNncmQ/dj0zJnVpbj04MTExNjczNzkmc2l0ZT1xcSZtZW51PXllcw==" target="_blank">点击这里</a>获取', 'alipay'),
                'css'         => 'width:400px'
            ),
            'secure_key' => array(
                'title'       => __('商户密钥', 'alipay'),
                'type'        => 'text',
                'description' => __('请输入您的商户密钥<br />如果您还没有商户ID，请<a href="https://www.mobanzhu.com/go/aHR0cDovL3dwYS5xcS5jb20vbXNncmQ/dj0zJnVpbj04MTExNjczNzkmc2l0ZT1xcSZtZW51PXllcw==" target="_blank">点击这里</a>获取', 'alipay'),
                'css'         => 'width:400px'
            ),
            'alipay_account' => array(
                'title'       => __('收款支付宝', 'alipay'),
                'type'        => 'text',
           ...
           /* 'order_prefix' => array(
            ...
            'description_notice' => array(
                'title'       => __('通知网址', 'alipay'),
                'type'        => 'text',
                'description' => __('这里是免签辅助软件上所填写的通知，请复制粘贴到软件上。', 'alipay'),
                'default'     => 'http://网址/wc-api/WC_Alipay/',
                'desc_tip'    => true,
            ),
    ...
    function get_alipay_args( $order ) {
        ...
        $alipay_args = array(
            ...
            "optEmail"      => $this->alipay_account,
            "title"      => $this->order_prefix . ltrim( $order->get_order_number(), '#' ),
            //"subject"           => $subject,
            "payAmount"             => $total_fee,
            //"quantity"          => 1,          
    ...
    function check_alipay_response() {
        ...
        @ob_clean();
		$alidirect_pid = $this->partnerID;
		$alidirect_key = $this->secure_key;
		...
		if(!preg_match("/^[0-9a-z_]{1,16}$/i",$title)){exit("fail");}
			
			global $wpdb;

            if( is_numeric( $title ) ){
                if( !empty( $this->order_prefix ) ){
                    $order_id  = (int) str_replace( $this->order_prefix, '', $title );
                } else {
                    $order_id  = (int) $title;
                }
            } else {
		...
		if($act=="check")
		{
			$order = $wpdb->get_row("SELECT meta_value FROM ".$wpdb->prefix."postmeta WHERE post_id = ".$order_id." and meta_key = '_paid_date'");
			if($order->meta_value)
			{
				exit("success");
			}
        ...
        if(strtoupper(md5($alidirect_pid . $alidirect_key . $tradeNo . $Money . $title . $memo)) == strtoupper($Sign)){
			...
			$goods = $wpdb->get_row("SELECT meta_value FROM ".$wpdb->prefix."postmeta WHERE post_id = ".$order_id." and meta_key = '_order_total'");
			$amount= $goods->meta_value;
			if(number_format($amount,2)!=number_format($Money,2)){exit("fail");}
            $order->payment_complete();                   

            if( isset( $_POST['tradeNo'] ) && !empty( $_POST['tradeNo'] ) ){
                update_post_meta( $order_id, 'Alipay Trade No.', wc_clean( $_POST['tradeNo'] ) );
            }  
            $this->successful_request( $_POST, $order_id );//此函数中包含输出success
