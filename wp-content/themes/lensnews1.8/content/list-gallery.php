<?php global $salong; ?>
<?php if($salong['switch_gallery_content']==0) { ?>
<article class="gallery_grid<?php wow(); ?>">
    ...
    <section class="gallery_popup">
        ...
        <span>
            <?php _e( '作者：', 'salong' ); ?><?php if(get_post_meta($post->ID, "author", true)){ $author = get_post_custom_values( "author"); echo $author[0]; } else { ?><?php the_author_nickname(); ?><?php } ?>
        </span>
        <a href="<?php if(get_post_meta($post->ID, "thumb", true)){ $thumb = get_post_custom_values( "thumb"); echo $thumb[0]; } else if( has_post_thumbnail() ){ ?><?php $large_image_url = wp_get_attachment_image_src( get_post_thumbnail_id($post->ID), 'full'); echo $large_image_url[0]; } else { ?><?php echo get_content_first_image(get_the_content()); ?><?php } ?>" class="btn view" data-fancybox="gallery" title="<?php the_title(); ?>"><i class="icon-search-1"></i></a>
        <a href="<?php the_permalink() ?>" class="btn link"<?php echo new_open_link(); ?>><i class="icon-link" title="<?php _e( '查看画廊', 'salong' ); ?>"></i></a>
    </section>
</article>
<?php } else { ?>
